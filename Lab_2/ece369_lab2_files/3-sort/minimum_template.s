...
# functionality   
# Changed branching to happen if less than or equal to instead of greater than or equal to
...
# We implemented the assembly instructions as the comments specified
#
...
loopMaxIndex:
    bge     $t1,$a1,doneMaxIndex    # i>=n ?
    mul     $t2, $t1, 4     # $t2 = $t1 * 4
    add     $t2,$t2,$a0
    lw      $t2, 0($t2)     # $t2 = V[i]
    ble     $t2,$t0,nextMaxIndex    # V[i] >= min ?
    add     $t0,$t2,$0      # min=V[i]
    add     $t3,$t1,$0      # max_index=max
nextMaxIndex:
    addi    $t1,$t1,1       # i++
    j       loopMaxIndex            # Loop back
doneMaxIndex: 
    add     $v0,$t3,$0      # return min
    jr      $ra
